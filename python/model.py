...
def annotate(results :dict,image:Image) ->None:
       ...
       for score, label, box in zip(results["scores"], results["labels"], results["boxes"]):
          ...
          text = f"{Model.id2label(label)}: {round(score,3)*100}%"
...
class AnntReq(BaseModel):
    raw_imgs : Optional[bytes] | Optional[str] = None
...
async def antt(file: Annotated[UploadFile, Form()], scores: Annotated[str, Form()]):
    print(file)
    print(scores)
    ...
    image.show()
        # resp = dict()
        # if req.raw_imgs:
        #     image=  from_bytes(req.raw_imgs)
        # else :
        #     assert req.urls
        #     image = from_url(req.urls)

        # annotate(req.meta,image)
        # return dump_bytes(image)
    # return {"ok":True}


@app.get("/infer")
async def root(req : Annotated[Request,Form()] ):
    images = []
